...
class MainWindow(QMainWindow):
    ...
    def get_lot(self):
        ...
        mycursor.execute('SELECT license_plate FROM distance_data ORDER BY id DESC LIMIT 1')
        license_plate_data = mycursor.fetchone()

        mycursor.execute('SELECT distance FROM distance_data ORDER BY id DESC LIMIT 1')
        height_data = mycursor.fetchone()

        # ตรวจสอบข้อมูลป้ายทะเบียน
        if license_plate_data:
            api_noplate = license_plate_data[0]  # ดึงค่าป้ายทะเบียนจากฐานข้อมูล
        else:
            api_noplate = None  # กรณีไม่มีข้อมูล ให้เป็น None หรือค่าอื่นตามต้องการ

        # ตรวจสอบข้อมูลความสูง
        if height_data:
            api_height = int(height_data[0])
        else:
            api_height = 0
        ...
        try:
            for _ in range(8):
                if car_height >= 190:
                    ...
                    for data_row in data_to_insert_history:
                        ...
                        license_plate_in_list = data_row[5]
                        mycursor.execute('''
                            INSERT INTO history
                            (his_id, card_id, height, license_plate, lot_id, time_in, time_out)
                            VALUES (NULL, %s, %s, %s, %s, %s, NULL)
                        ''', (card_id_in_list, height_in_list, lp_in_list, lot_id_in_list, timestamp_in_list, license_plate_in_list))
                    ...
                    if len(nz[removed_element]) == 1:
                        # continue
                        if len(oz[removed_element]) == 0:
                            continue
                        else:
                            parking_lot = oz[removed_element].pop(1)
                    else:
                        parking_lot = nz[removed_element].pop(0)
            ...
            self.ui.accept_status_label.setText(" ") #มีปัญหาโทรหาหมอ โจ้
